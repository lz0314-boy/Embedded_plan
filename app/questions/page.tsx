...
export default function QuestionsPage() {
  ...
  return <><div className="eyebrow">题库</div><h1>面试题库</h1><p className="muted">按关键词查找短答案、深入解释、平台边界和来源状态。当前共 {questions.length} 道匹配题目。</p><div className="field" style={{ maxWidth: 560, marginTop: 24 }}><label htmlFor="question-search">搜索问题、平台或源码符号</label><input id="question-search" value={query} onChange={(event) => changeQuery(event.target.value)} placeholder="例如 PendSV、fork、设备树" /></div><div className="list question-results">{visibleQuestions.map((item) => <article className="panel" key={item.id}><div className="status pending">待核验</div><h2 style={{ margin: "10px 0 6px" }}><Link href={`/learn/${item.slug}/`}>{item.title}</Link></h2><p className="muted">{item.pillar} · {item.module} · {item.platforms.join(" / ")}</p><p>{item.body}</p></article>)}</div><div className="button-row pagination"><button className="button" disabled={currentPage === 0} onClick={() => setPage((value) => Math.max(0, value - 1))}>上一页</button><span className="muted">第 {currentPage + 1} / {pageCount} 页</span><button className="button" disabled={currentPage + 1 >= pageCount} onClick={() => setPage((value) => Math.min(pageCount - 1, value + 1))}>下一页</button></div></>;
}
