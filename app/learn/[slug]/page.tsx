...
export default async function ContentPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><div className="eyebrow">{item.pillar} / {item.module}</div><h1>{item.title}</h1><p className="muted">{item.scope} · {item.platforms.join(" / ")} · <span className={`status ${verified ? "verified" : "pending"}`}>{labelStatus(item.status, item.verifiedAt)}</span></p><p className="muted">这份内容可以直接用于学习；状态表示编辑和证据覆盖程度，不是访问权限。板级事实若未绑定匹配资料，会单独标注“待板级核验”。</p><LearnActions contentId={item.id} /><article className="content-body" dangerouslySetInnerHTML={{ __html: item.html }} /><section className="panel" style={{ maxWidth: 760, marginTop: 32 }}><h2 style={{ marginTop: 0 }}>来源与边界</h2><ul>{item.sourceIds.map((id) => { const source = getSourceById(id); return <li key={id}>{source ? <Link href={source.url} target="_blank" rel="noreferrer">{source.title}</Link> : id}</li>; })}</ul><p className="muted">未完成固定版本和人工逐条核验的内容保持“待核验”；这不阻止阅读、做题或记录学习数据。</p></section><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: serializeJsonLd(contentJsonLd(item, sources)) }} /></>;
}
